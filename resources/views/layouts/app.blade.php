...
<head>
    ...
    <link href="https://fonts.bunny.net/css?family=syne:600,700,800|dm-sans:ital,wght@0,300;0,400;0,500;0,600;1,400&display=swap" rel="stylesheet">
...
<div x-data="{ sidebarOpen: false }" class="flex h-screen overflow-hidden" style="background:#f5f6fa;">
    ...
    >
        {{-- Logo --}}
        <div class="flex items-center gap-3 h-16 px-5 shrink-0" style="border-bottom:1px solid rgba(255,255,255,0.07);">
            <a href="{{ route('dashboard') }}" class="flex items-center gap-2.5 flex-1 min-w-0">
                <img src="/img/logo.png" alt="{{ config('app.name') }}" class="h-7 w-auto brightness-0 invert shrink-0">
                <span class="text-base font-bold text-white truncate" style="font-family:'Syne',sans-serif;letter-spacing:-0.01em;">InvoiceKit</span>
            </a>
            <button
                @click="sidebarOpen = false"
                class="lg:hidden p-1.5 rounded-lg shrink-0"
        ...
        <nav class="flex-1 px-3 py-5 space-y-0.5 overflow-y-auto">
            <p class="px-3 mb-2 text-[10px] font-semibold uppercase tracking-[0.13em] select-none" style="color:rgba(255,255,255,0.18);">Navigation</p>
            <x-sidebar-link :href="route('dashboard')" :active="request()->routeIs('dashboard')" icon="grid">{{ __('Dashboard') }}</x-sidebar-link>
            ...
            <x-sidebar-link :href="route('projects.index')" :active="request()->routeIs('projects.*')" icon="folder">{{ __('Projects') }}</x-sidebar-link>
            <x-sidebar-link :href="route('invoices.index')" :active="request()->routeIs('invoices.*')" icon="document">{{ __('Invoices') }}</x-sidebar-link>
            <x-sidebar-link :href="route('billing.index')" :active="request()->routeIs('billing.*')" icon="credit-card">{{ __('Billing') }}</x-sidebar-link>
        </nav>
        ...
        <div class="px-3 pb-2 shrink-0" style="border-top:1px solid rgba(255,255,255,0.07);">
            <div x-data="{ langOpen: false, langSearch: '' }" class="relative pt-2">
                ...
                >
                    ...
                    <span class="flex-1 text-left text-xs font-medium truncate">{{ $currentLocaleData['name'] }}</span>
                    <span class="text-[9px] font-bold uppercase tracking-widest shrink-0" style="color:rgba(255,255,255,0.2);">{{ strtoupper($currentLocale) }}</span>
                    <svg class="w-3 h-3 shrink-0 transition-transform duration-150" :class="langOpen ? 'rotate-180' : ''" fill="none" stroke="currentColor" style="color:rgba(255,255,255,0.2);" viewBox="0 0 24 24">
                ...
                >
                    <div class="p-2" style="border-bottom:1px solid rgba(255,255,255,0.06);">
                        <div class="relative">
                            <svg class="absolute left-2.5 top-1/2 -translate-y-1/2 w-3 h-3 pointer-events-none" style="color:rgba(255,255,255,0.25);" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @foreach ($supportedLanguages as $locale)
                            ...
                            >
                                ...
                                >
                                    ...
                                    @if ($isActive)
                                        <svg class="ml-auto w-3 h-3 shrink-0" style="color:#f59e0b;" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div class="px-3 pb-4 shrink-0">
            <div class="flex items-center gap-3 px-3 py-2.5 rounded-xl" style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.07);">
                ...
                <div class="flex-1 min-w-0">
                    <p class="text-xs font-semibold truncate" style="color:rgba(255,255,255,0.85);">{{ Auth::user()->name }}</p>
                    <p class="text-[10px] truncate mt-0.5" style="color:rgba(255,255,255,0.3);">{{ Auth::user()->email }}</p>
                </div>
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header
            class="lg:hidden flex items-center gap-3 h-14 px-4 shrink-0"
            style="background:#0f1117;border-bottom:1px solid rgba(255,255,255,0.07);"
        >
            <button
                @click="sidebarOpen = true"
                class="p-2 -ml-1 rounded-lg transition-all"
            ...
            <a href="{{ route('dashboard') }}" class="flex items-center gap-2 select-none">
                ...
                <span class="text-[15px] font-bold text-white" style="font-family:'Syne',sans-serif;">InvoiceKit</span>
            </a>
        </header>
...
>
    <p class="text-sm leading-relaxed max-w-2xl" style="color:rgba(255,255,255,0.5);">
        ...
        <a href="{{ url('/privacy') }}" class="ml-1 underline" style="color:rgba(255,255,255,0.7);">Privacy Policy</a>
    </p>
    <button
