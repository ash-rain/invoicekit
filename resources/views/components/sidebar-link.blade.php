...
@if ($active)
    <a href="{{ $href }}" {{ $attributes->merge(['class' => 'flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold']) }}
        style="background:rgba(245,158,11,0.12);color:#fbbf24;box-shadow:inset 0 0 0 1px rgba(245,158,11,0.18);">
        <svg style="width:17px;height:17px;flex-shrink:0;" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
            <path d="{{ $path }}"/>
        </svg>
        {{ $slot }}
    </a>
@else
    <a href="{{ $href }}" {{ $attributes->merge(['class' => 'flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all']) }}
