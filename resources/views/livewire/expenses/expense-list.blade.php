    ...
    <div class="bg-white rounded-2xl overflow-hidden" style="border:1px solid #eaecf0;">
        ...
        @if ($expenses->isEmpty())
            <div class="p-12 text-center">
                ...
                <p class="text-xs text-gray-400 mt-1">{{ __('Add your first expense to start tracking spending.') }}</p>
            </div>
        @else
            <table class="min-w-full">
                <thead style="background:#fafafa;border-bottom:1px solid #f3f4f6;">
                    <tr>
                        ...
                        <th class="px-5 py-3 text-center text-[10px] font-bold text-gray-400 uppercase tracking-wider">{{ __('Billable') }}</th>
                        <th class="px-5 py-3"></th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($expenses as $expense)
                        <tr class="border-t border-[#f3f4f6] hover:bg-[#fafafa] transition" wire:key="expense-{{ $expense->id }}">
                            <td class="px-5 py-4">
                            ...
                            <td class="px-5 py-4">
                                <span class="inline-flex items-center px-2.5 py-0.5 text-xs font-semibold rounded-full bg-gray-100 text-gray-600 capitalize">
                                    {{ __($expense->category) }}
                                </span>
                            </td>
                            <td class="px-5 py-4">
                            ...
                            <td class="px-5 py-4 text-right">
                                <span class="text-sm font-semibold text-gray-800">{{ formatCurrency($expense->currency, (float) $expense->amount) }}</span>
                            </td>
                            <td class="px-5 py-4 text-center">
                                @if ($expense->billable)
                                    <span class="inline-flex items-center px-2.5 py-0.5 text-xs font-bold rounded-full bg-blue-100 text-blue-700">{{ __('Billable') }}</span>
                                @else
