        ...
        @if ($recurringInvoices->isEmpty())
            <div class="p-12 text-center">
                ...
                <p class="text-xs text-gray-400 mt-1">
                    {{ __('Create a template to auto-generate invoices on a schedule.') }}</p>
            </div>
        @else
                ...
                <tbody>
                    @foreach ($recurringInvoices as $recurring)
                        <tr class="border-t border-[#f3f4f6] hover:bg-[#fafafa] transition"
                            wire:key="recurring-{{ $recurring->id }}">
                            <td class="px-5 py-4">
                            ...
                            <td class="px-5 py-4">
                                <span
                                    class="text-sm text-gray-600">{{ $recurring->next_send_date->format('d M Y') }}</span>
                            </td>
                            <td class="px-5 py-4 text-right">
                                <span
                                    class="text-sm font-semibold text-gray-800">{{ formatCurrency($recurring->currency, (float) $recurring->total) }}</span>
                            </td>
