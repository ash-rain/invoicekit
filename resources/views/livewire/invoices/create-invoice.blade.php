<div class="p-6 max-w-4xl mx-auto">
    ...
    <form wire:submit="save" class="space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-[#eaecf0] p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            {{-- Poland KSeF reference number --}}
            @if ($sellerCountry === 'PL')
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('KSeF number (optional)') }}</label>
                    <input wire:model="ksefId" type="text"
                        class="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 @error('ksefId') border-red-400 @enderror" />
                    @error('ksefId')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                    <p class="mt-1 text-xs text-gray-400">{{ __('If this invoice has been issued through KSeF, enter its reference number here — it will be included in the bank transfer payment reference shown to your client.') }}</p>
                </div>
            @endif
