[nav_features]
Jellemzők

[nav_pricing]
Árak

[nav_login]
Bejelentkezés

[nav_cta]
Kezdj ingyenesen

[hero_badge]
🇪🇺 Európai szabadúszóknak tervezve

[hero_headline]
Számlázz európaian.
Kövesd az időt profiként.

[hero_subheadline]
Az egyetlen eszköz, amelyre az EU szabadúszóinak szükségük van — az időkövetéstől a jogszerű ÁFA-megfelelő számlákig. Automatikusan kezeli a fordított adózást, az OSS-t és az összes 24 EU ÁFA-kulcsot.

[hero_cta_primary]
Kezdj ingyenesen — kártya nélkül

[hero_cta_secondary]
Funkciók megtekintése ↓

[hero_trust_1]
Ingyenes kezdés

[hero_trust_2]
Bankkártya nem szükséges

[hero_trust_3]
Bármikor lemondható

[features_tag]
Funkciók

[features_title]
Minden egy helyen

[features_subtitle]
Hagyj fel a táblázatok és különálló alkalmazások egyensúlyozásával. Az InvoiceKit minden lépést kezel az első számlázható perctől az utolsó begyűjtött euróig.

[f1_title]
Időkövetés

[f1_desc]
Start/stop időzítő projektenként. Kézi bevitel a múltbeli munkákhoz. Napi naplók, heti összefoglalók és havi jelentések a teljes számlázható órákkal.

[f2_title]
Számla-készítő

[f2_desc]
Generálj professzionális PDF számlákat másodpercek alatt. Automatikus sorszámozás, jogi formázás, az összes kötelező EU mezővel, beleértve a teljes ÁFA-részletezést.

[f3_title]
EU ÁFA-motor

[f3_desc]
Automatikus ÁFA minden EU-s forgatókönyvhöz: általános kulcs, fordított adózás (B2B), OSS (B2C EU-fogyasztók) és 0% mentes az EU-n kívüli vevők számára.

[f4_title]
Ügyfélkezelés

[f4_desc]
Tárold az ügyfeleket országgal, adószámmal és pénznemmel. Országzászlók, azonnali keresés és teljes számlázási előzmények ügyfelenként.

[f5_title]
Ismétlődő számlák

[f5_desc]
Állítson be heti, havi, negyedéves vagy éves számlákat. Az InvoiceKit automatikusan klónozza és elküldi azokat — nincs kézi munka, nincsenek elmaradt számlázási ciklusok.

[f6_title]
Ügyfélportál

[f6_desc]
Osszon meg bármely ügyféllel egy biztonságos, tokenizált hivatkozást. Megtekinthetik a számlát, letölthetik a PDF-et, és kártyával fizethetnek online a Stripe segítségével — a pénz közvetlenül az Ön bankszámlájára érkezik. Vagy ossza meg IBAN-ját az ingyenes banki átutalásokhoz. Bejelentkezés nélkül.

[f7_title]
Bármilyen eszközön működik

[f7_desc]
Adja hozzá az InvoiceKitet telefonja, táblagépe vagy számítógépe kezdőképernőjéhez. Natív alkalmazásként telepít, böngésző nélkül indul és offline is működik — így akárhol rögzíthet időt vagy ellenőrizhet számlákat.

[f8_title]
Kiadáskövetés

[f8_desc]
Rögzítse az üzleti kiadásokat bizonylatokkal, kategóriákkal és szállítói adatokkal. Szűrjön dátum vagy kategória szerint, exportálja CSV-be, és kövesse havi kiadásait bevételei mellett.

[f9_title]
Több pénznem

[f9_desc]
Számlázzon EUR, USD, BGN, RON, PLN, CZK vagy HUF-ban. Minden ügyfél tárolja a preferált pénznemét — automatikusan kitöltődik minden új számláján.

[how_tag]
Hogyan működik

[how_title]
A követett másodpercektől a kifizetett számlákig

[how_subtitle]
Három egyszerű lépés. Ez minden, ami kell.

[s1_title]
Kövesd az idődet

[s1_desc]
Kattints a Start gombra egy projektnél. Dolgozz. Kattints a Stop gombra. Minden másodpercet automatikusan rögzít — nincs táblázat, nincs felejtés.

[s2_title]
Generálj számlát

[s2_desc]
Válassz ügyfelet, jelöld ki a számlázandó időbejegyzéseket. Az InvoiceKit összeállítja a számlát a helyes ÁFÁval, jogi formázással és márkáddal.

[s3_title]
Kapj fizetést

[s3_desc]
Küldje el egy kattintással. Ügyfele kártyával fizet a Stripe segítségével, vagy ingyenes banki átutalással. A pénz közvetlenül az Ön bankszámlájára érkezik. Kész.

[vat_tag]
EU-megfelelőség

[vat_title]
ÁFA minden alkalommal helyesen

[vat_desc]
Az EU ÁFA-jog összetett. Az InvoiceKit automatikusan kezel minden forgatókönyvet — általános kulcsok, fordított adózás a B2B ügyfelek számára, OSS az EU-fogyasztók számára és 0% az EU-n kívüli vevők számára.

[vat_r1_label]
Számlázás ugyanabban az országban

[vat_r1_sub]
A helyi általános ÁFA-kulcs automatikusan alkalmazva

[vat_r2_label]
EU B2B (Fordított adózás)

[vat_r2_sub]
0% ÁFA — a vevő elszámolja az ÁFÁt a 2006/112/EK irányelv 44. cikke alapján

[vat_r3_label]
EU B2C (OSS szabályok)

[vat_r3_sub]
Az eladó kulcsa alkalmazva (vagy OSS kulcs ahol releváns)

[vat_r4_label]
EU-n kívüli vevő

[vat_r4_sub]
0% ÁFA — az EU ÁFA hatályán kívül, mentes

[vat_rates_title]
Támogatott ÁFA-kulcsok

[pricing_tag]
Árak

[pricing_title]
Egyszerű, becsületes árak

[pricing_subtitle]
Kezdd ingyenesen. Frissíts, amikor a vállalkozásod nő. Nincs szerződés, bármikor lemondható.

[p_free_name]
Ingyenes

[p_free_price]
€0

[p_free_period]
örökre

[p_free_desc]
Minden, amire szükséged van a kezdéshez.

[p_free_l1]
Legfeljebb 3 ügyfél

[p_free_l2]
Havi 5 számla

[p_free_l3]
Időkövetés

[p_free_l4]
PDF számla exportálása

[p_free_cta]
Kezdj ingyenesen

[p_starter_name]
Starter

[p_starter_price]
€9

[p_starter_period]
havonta

[p_starter_desc]
Növekvő szabadúszóknak.

[p_starter_l1]
Korlátlan számú ügyfél

[p_starter_l2]
Havi 20 számla

[p_starter_l3]
Időkövetés

[p_starter_l4]
Összes EU ÁFA-szabály

[p_starter_l5]
Fizetési emlékeztetők

[p_starter_cta]
Starter választása

[p_pro_name]
Pro

[p_pro_price]
€29

[p_pro_period]
havonta

[p_pro_desc]
Komoly szabadúszóknak.

[p_pro_l1]
Minden korlátlan

[p_pro_l2]
Ismétlődő számlák

[p_pro_l3]
Ügyfélportál

[p_pro_l4]
Peppol / e-Számlázás (UBL 2.1)

[p_pro_l5]
ÁFA-mentesség mód (mind a 27 EU-ország)

[p_pro_l6]
Összes EU ÁFA-szabály

[p_pro_l7]
Online fizetések Stripe-on keresztül (2% díj)

[p_pro_cta]
Pro-ra váltás

[p_popular]
Legnépszerűbb

[faq_tag]
GYIK

[faq_title]
Gyakran ismételt kérdések

[faq1_q]
Az InvoiceKit valóban megfelel az EU ÁFA-szabályainak?

[faq1_a]
Igen. Az InvoiceKit automatikusan kiszámítja a helyes ÁFÁt minden számlához az ország, az ügyfél országa és érvényes EU adószám alapján. Helyesen kezeli az általános kulcsokat, a fordított adózást, az OSS-t és az EU-n kívüli mentességeket.

[faq2_q]
Mely EU-tagállamok támogatottak?

[faq2_a]
Mind a 27 EU-tagállam. Az ÁFA-kulcsok rögzítve vannak BG, DE, FR, RO, PL, CZ, IT, ES, NL, PT, AT, BE, HR, HU, SE és az összes többi esetén. A kulcsok frissülnek az EU-s jogszabályok változásakor.

[faq3_q]
Támogatja az InvoiceKit az ismétlődő számlákat?

[faq3_a]
Igen. Bármelyik számlát átállíthatja ismétlődőre heti, havi, negyedéves vagy éves intervallumokkal. Az InvoiceKit automatikusan klónozza és kiállítja a számlát a határnapon — önnek semmit sem kell tennie.

[faq4_q]
Hogyan működik az időkövetés?

[faq4_a]
Hozz létre projekteket ügyfelekkel és órabérekkel. Használd a beépített start/stop időzítőt munka közben. Az összes követett idő el van mentve és egy kattintással közvetlenül számlasorrá alakítható.

[faq5_q]
Van ingyenes próbaidőszak?

[faq5_a]
Igen. Minden új fiók 14 napos Pro próbaidőszakkal kezd — hitelkártya nem szükséges. Az első naptól teljes Pro hozzáférést kap. A próbaidőszak után az ingyenes csomaggal folytatja, hacsak nem dönt az upgrade mellett.


[pay_tag]
Online fizetések

[pay_title]
Fogadja a fizetéseket közvetlenül. Mindig.

[pay_desc]
Csatlakoztassa Stripe-fiókját, és alakítson minden számlát fizetési hivatkozássá. Az ügyfelek kártyával fizetnek — a pénz közvetlenül az Ön bankszámlájára érkezik, nem közvetítőhöz. Az IBAN-on keresztüli banki átutalások mindig ingyenesek.

[pay_r1_label]
Stripe Connect — közvetlenül az Ön bankjába

[pay_r1_sub]
Stripe Connect Expresst használunk. Ügyfele kártyás fizetése egyenesen az Ön bankszámlájára kerül. Az InvoiceKit soha nem tartja vissza a pénzét.

[pay_r2_label]
Ügyfélportál — bejelentkezés nélkül

[pay_r2_sub]
Minden számla kap egy biztonságos tokenizált hivatkozást. Az ügyfél bármely böngészőben megnyitja, látja a számlát, és egy kattintással fizet.

[pay_r3_label]
Csak 2% kártyás fizetésre — banki átutalások ingyenesek

[pay_r3_sub]
2%-os platformdíj csak kártyás fizetéskor érvényes. Az IBAN-on keresztüli banki átutalások teljesen ingyenesek.

[faq6_q]
Hogyan működnek az online fizetések?

[faq6_a]
Csatlakoztassa Stripe-fiókját a Beállítások → Fizetések menüpontban. Az InvoiceKit minden számlához biztonságos fizetési hivatkozást generál. Ügyfele rákattint a Fizess most gombra, és kártyával fizet. A pénz közvetlenül az Ön bankszámlájára érkezik, az InvoiceKit soha nem tartja vissza. Kártyás fizetésnél 2%-os platformdíj érvényes. Az IBAN-on keresztüli banki átutalások mindig ingyenesek.

[cta_title]
Készen áll számlázni európaian?

[cta_subtitle]
Csatlakozz azokhoz az európai szabadúszókhoz, akik abbahagyták a táblázatokkal való küzdelmet és elkezdtek időben fizetést kapni.

[cta_button]
Kezdj ingyenesen ma →

[footer_tagline]
EU-megfelelő számlázás európai szabadúszóknak.

[footer_privacy]
Adatvédelmi irányelvek

[footer_terms]
Szolgáltatási feltételek

[footer_copy]
© 2026 InvoiceKit. Minden jog fenntartva.
