[nav_features]
Značajke

[nav_pricing]
Cijene

[nav_login]
Prijava

[nav_cta]
Počni besplatno

[hero_badge]
🇪🇺 Stvoreno za europske freelancere

[hero_headline]
Fakturiraj kao Europljanin.
Prati vrijeme kao profesionalac.

[hero_subheadline]
Jedini alat koji EU freelanceri trebaju — od praćenja vremena do zakonski usklađenih PDV faktura. Automatski obrađuje prijenos porezne obveze, OSS i svih 24 PDV stopa u EU.

[hero_cta_primary]
Počni besplatno — bez kartice

[hero_cta_secondary]
Pogledaj značajke ↓

[hero_trust_1]
Besplatan početak

[hero_trust_2]
Bez kreditne kartice

[hero_trust_3]
Otkazi bilo kada

[features_tag]
Značajke

[features_title]
Sve na jednom mjestu

[features_subtitle]
Prestani žonglirati s proračunskim tablicama i zasebnim aplikacijama. InvoiceKit upravlja svakim korakom od prve naplative minute do posljednjeg naplaćenog eura.

[f1_title]
Praćenje vremena

[f1_desc]
Start/stop tajmer po projektu. Ručni unos za prošle radove. Dnevni dnevnici, tjedni sažetci i mjesečni izvještaji s ukupnim naplativim satima.

[f2_title]
Kreator faktura

[f2_desc]
Generiraj profesionalne PDF fakture za nekoliko sekundi. Automatska numeracija, zakonski format, sa svim obaveznim EU poljima uključujući potpuni prikaz PDV-a.

[f3_title]
EU PDV motor

[f3_desc]
Automatski PDV za svaki EU scenarij: standardna stopa, prijenos porezne obveze (B2B), OSS (B2C EU potrošači) i 0% izuzeto za kupce izvan EU.

[f4_title]
Upravljanje klijentima

[f4_desc]
Pohrani klijente s državom, PDV brojem i valutom. Oznake država, trenutačna pretraga i potpuna povijest fakturiranja po klijentu.

[f5_title]
Ponavljajući računi

[f5_desc]
Postavi tjedne, mjesečne, tromjesečne ili godišnje račune. InvoiceKit ih automatski klonira i šalje — bez ručnog rada, bez propuštenih ciklusa naplate.

[f6_title]
Klijentski portal

[f6_desc]
Podijelite sigurnu tokeniziranu vezu s bilo kojim klijentom. Mogu pregledati fakturu, preuzeti PDF i platiti online karticom putem Stripea — sredstva idu izravno na vaš bankovni račun. Ili podijelite IBAN za besplatne bankovne prijenose. Prijava nije potrebna.

[f7_title]
Radi na svakom uređaju

[f7_desc]
Dodajte InvoiceKit na početni zaslon telefona, tableta ili računala. Instalira se kao nativna aplikacija, pokreće bez preglednika i radi offline kako biste mogli bilježiti vrijeme ili provjeravati fakture s bilo kojeg mjesta.

[f8_title]
Praćenje troškova

[f8_desc]
Zabilježite poslovne troškove s računima, kategorijama i podacima o dobavljaču. Filtrirajte po datumu ili kategoriji, izvezite u CSV i pratite mjesečni pregled troškova uz prihode.

[f9_title]
Više valuta

[f9_desc]
Fakturirajte u EUR, USD, BGN, RON, PLN, CZK ili HUF. Svaki klijent pohranjuje željenu valutu — automatski se popunjava na svakom novom računu.

[how_tag]
Kako funkcionira

[how_title]
Od praćenih sekundi do plaćene fakture

[how_subtitle]
Tri jednostavna koraka. To je sve što treba.

[s1_title]
Prati svoje vrijeme

[s1_desc]
Klikni Start za projekt. Radi. Klikni Stop. Svaka sekunda se automatski bilježi — bez tablica, bez zaboravljanja.

[s2_title]
Generiraj fakturu

[s2_desc]
Odaberi klijenta, označi time unose za naplatu. InvoiceKit izrađuje fakturu s ispravnim PDV-om, zakonskim formatom i tvojim brendom.

[s3_title]
Budi plaćen

[s3_desc]
Pošaljite jednim klikom. Vaš klijent plaća karticom putem Stripea ili besplatnim bankovnim prijenosom. Sredstva stižu izravno na vaš bankovni račun. Gotovo.

[vat_tag]
EU usklađenost

[vat_title]
PDV ispravno svaki put

[vat_desc]
EU PDV pravo je složeno. InvoiceKit automatski obrađuje svaki scenarij — standardne stope, prijenos porezne obveze za B2B klijente, OSS za EU potrošače i 0% za kupce izvan EU.

[vat_r1_label]
Fakturiranje u istoj državi

[vat_r1_sub]
Lokalna standardna PDV stopa automatski primijenjena

[vat_r2_label]
EU B2B (Prijenos porezne obveze)

[vat_r2_sub]
0% PDV — kupac obračunava PDV prema čl. 44 Dir. 2006/112/EZ

[vat_r3_label]
EU B2C (OSS pravila)

[vat_r3_sub]
Primijenjena stopa prodavatelja (ili OSS stopa gdje je primjenjivo)

[vat_r4_label]
Kupac izvan EU

[vat_r4_sub]
0% PDV — izvan opsega EU PDV-a, izuzeto

[vat_rates_title]
Podržane PDV stope

[pricing_tag]
Cijene

[pricing_title]
Jednostavne, poštene cijene

[pricing_subtitle]
Počni besplatno. Nadogradi kad tvoj posao raste. Bez ugovora, otkazi bilo kada.

[p_free_name]
Besplatno

[p_free_price]
€0

[p_free_period]
zauvijek

[p_free_desc]
Sve što trebaš za početak.

[p_free_l1]
Do 3 klijenta

[p_free_l2]
5 faktura mjesečno

[p_free_l3]
Praćenje vremena

[p_free_l4]
PDF izvoz faktura

[p_free_cta]
Počni besplatno

[p_starter_name]
Starter

[p_starter_price]
€9

[p_starter_period]
mjesečno

[p_starter_desc]
Za rastuće freelancere.

[p_starter_l1]
Neograničen broj klijenata

[p_starter_l2]
20 faktura mjesečno

[p_starter_l3]
Praćenje vremena

[p_starter_l4]
Sva EU PDV pravila

[p_starter_l5]
Podsjetnici plaćanja

[p_starter_cta]
Odaberi Starter

[p_pro_name]
Pro

[p_pro_price]
€29

[p_pro_period]
mjesečno

[p_pro_desc]
Za ozbiljne freelancere.

[p_pro_l1]
Sve neograničeno

[p_pro_l2]
Ponavljajuće fakture

[p_pro_l3]
Klijentski portal

[p_pro_l4]
Peppol / e-Fakturiranje (UBL 2.1)

[p_pro_l5]
Način oslobođenja od PDV-a (svih 27 zemalja EU)

[p_pro_l6]
Sva EU PDV pravila

[p_pro_l7]
Online plaćanja putem Stripea (naknada 2%)

[p_pro_cta]
Idi na Pro

[p_popular]
Najpopularnije

[faq_tag]
FAQ

[faq_title]
Često postavljana pitanja

[faq1_q]
Je li InvoiceKit zaista u skladu s EU PDV-om?

[faq1_a]
Da. InvoiceKit automatski izračunava ispravan PDV za svaku fakturu na temelju tvoje države, države klijenta i ima li valjani EU PDV broj. Ispravno obrađuje standardne stope, prijenos porezne obveze, OSS i scenarije izuzimanja izvan EU.

[faq2_q]
Koje EU države su podržane?

[faq2_a]
Svih 27 država članica EU. PDV stope su ugrađene za BG, DE, FR, RO, PL, CZ, IT, ES, NL, PT, AT, BE, HR, HU, SE i sve ostale. Stope se ažuriraju kad se EU zakonodavstvo mijenja.

[faq3_q]
Podržava li InvoiceKit ponavljajuće račune?

[faq3_a]
Da. Možeš pretvoriti bilo koji račun u ponavljajući s tjednim, mjesečnim, tromjesečnim ili godišnjim intervalima. InvoiceKit automatski klonira i izdaje račun na datum dospijeća — ne moraš ništa dirati.

[faq4_q]
Kako funkcionira praćenje vremena?

[faq4_a]
Kreiraj projekte s klijentima i satnicama. Koristi ugrađeni start/stop tajmer dok radiš. Sve praćeno vrijeme se pohranjuje i jednim klikom može se izravno pretvoriti u stavke fakture.

[faq5_q]
Postoji li besplatno probno razdoblje?

[faq5_a]
Da. Svi novi računi počinju s 14-dnevnim Pro probnim razdobljem — nije potrebna kreditna kartica. Imaš puni Pro pristup od prvog dana. Nakon probnog razdoblja ostaneš na besplatnom planu ako ne odabereš nadogradnju.


[pay_tag]
Online plaćanja

[pay_title]
Primajte plaćanja izravno. Uvijek.

[pay_desc]
Povežite Stripe račun i pretvorite svaku fakturu u link za plaćanje. Klijenti plaćaju karticom — sredstva idu izravno na vaš bankovni račun, ne posredniku. Bankovni prijenosi putem IBAN-a uvijek su besplatni.

[pay_r1_label]
Stripe Connect — izravno na vaš račun

[pay_r1_sub]
Koristimo Stripe Connect Express. Plaćanje karticom vašeg klijenta ide izravno na vaš bankovni račun. InvoiceKit nikada ne zadržava vaš novac.

[pay_r2_label]
Klijentski portal — bez prijave

[pay_r2_sub]
Svaka faktura dobiva sigurnu tokeniziranu vezu. Vaš klijent je otvara u bilo kom pregledniku, vidi fakturu i plaća jednim klikom.

[pay_r3_label]
Samo 2% za kartična plaćanja — bankovni prijenosi besplatni

[pay_r3_sub]
Naknada platforme od 2% primjenjuje se samo za kartična plaćanja. Bankovni prijenosi putem IBAN-a potpuno su besplatni.

[faq6_q]
Kako funkcioniraju online plaćanja?

[faq6_a]
Povežite Stripe račun u Postavkama → Plaćanja. InvoiceKit generira sigurni link za plaćanje za svaku fakturu. Vaš klijent klikne Plati sada i plati karticom. Sredstva stižu izravno na vaš bankovni račun — InvoiceKit nikada ne zadržava vaš novac. Na kartična plaćanja primjenjuje se naknada platforme od 2%. Bankovni prijenosi putem IBAN-a uvijek su besplatni.

[cta_title]
Spreman fakturirati kao Europljanin?

[cta_subtitle]
Pridruži se europskim freelancerima koji su prestali se boriti s proračunskim tablicama i počeli biti plaćeni na vrijeme.

[cta_button]
Počni besplatno danas →

[footer_tagline]
EU usklađeno fakturiranje za europske freelancere.

[footer_privacy]
Pravila privatnosti

[footer_terms]
Uvjeti usluge

[footer_copy]
© 2026 InvoiceKit. Sva prava zadržana.
