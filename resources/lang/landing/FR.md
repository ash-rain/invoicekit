[nav_features]
Fonctionnalités

[nav_pricing]
Tarifs

[nav_login]
Connexion

[nav_cta]
Commencer gratuitement

[hero_badge]
🇪🇺 Conçu pour les freelances européens

[hero_headline]
Facturez à l'européenne.
Suivez le temps comme un pro.

[hero_subheadline]
Le seul outil dont les freelances européens ont besoin — du suivi du temps aux factures TVA conformes à la loi. Gère automatiquement l'autoliquidation, l'OSS et les 24 taux de TVA de l'UE.

[hero_cta_primary]
Commencer gratuitement — sans carte

[hero_cta_secondary]
Voir les fonctionnalités ↓

[hero_trust_1]
Démarrage gratuit

[hero_trust_2]
Sans carte bancaire

[hero_trust_3]
Résiliation à tout moment

[features_tag]
Fonctionnalités

[features_title]
Tout en un seul endroit

[features_subtitle]
Fini les tableurs et les applications séparées. InvoiceKit gère chaque étape, de votre première minute facturable à votre dernier euro encaissé.

[f1_title]
Suivi du temps

[f1_desc]
Minuteur start/stop par projet. Saisie manuelle pour le travail passé. Journaux quotidiens, résumés hebdomadaires et rapports mensuels avec les heures facturables totales.

[f2_title]
Créateur de factures

[f2_desc]
Générez des factures PDF professionnelles en quelques secondes. Numérotées automatiquement, formatées légalement, avec tous les champs UE obligatoires dont la ventilation complète de la TVA.

[f3_title]
Moteur TVA européen

[f3_desc]
TVA automatique pour chaque scénario UE : taux normal, autoliquidation (B2B), OSS (consommateurs B2C UE) et 0 % exonéré pour les acheteurs hors UE.

[f4_title]
Gestion des clients

[f4_desc]
Enregistrez les clients avec leur pays, numéro de TVA et devise. Drapeaux de pays, recherche instantanée et historique de facturation complet par client.

[f5_title]
Factures récurrentes

[f5_desc]
Configurez des factures hebdomadaires, mensuelles, trimestrielles ou annuelles. InvoiceKit les clone et les envoie automatiquement — aucun travail manuel, aucun cycle de facturation oublié.

[f6_title]
Portail client

[f6_desc]
Partagez un lien sécurisé et tokenisé avec n'importe quel client. Il peut consulter la facture, télécharger le PDF et payer en ligne via Stripe — sans connexion requise.

[how_tag]
Comment ça marche

[how_title]
Des secondes suivies à la facture payée

[how_subtitle]
Trois étapes simples. C'est tout ce qu'il faut.

[s1_title]
Suivez votre temps

[s1_desc]
Cliquez sur Démarrer pour un projet. Travaillez. Cliquez sur Arrêter. Chaque seconde est enregistrée automatiquement — pas de tableurs, pas d'oublis.

[s2_title]
Générez la facture

[s2_desc]
Sélectionnez un client, choisissez les saisies de temps à facturer. InvoiceKit crée la facture avec la TVA correcte, le formatage légal et votre image de marque.

[s3_title]
Soyez payé

[s3_desc]
Envoyez d'un clic. Votre client paie via Stripe. Vous voyez l'argent sur votre compte. C'est fait.

[vat_tag]
Conformité UE

[vat_title]
TVA correcte à chaque fois

[vat_desc]
La législation TVA de l'UE est complexe. InvoiceKit gère chaque scénario automatiquement — taux normaux, autoliquidation pour les clients B2B, OSS pour les consommateurs UE et 0 % pour les acheteurs hors UE.

[vat_r1_label]
Facturation dans le même pays

[vat_r1_sub]
Taux de TVA local standard appliqué automatiquement

[vat_r2_label]
UE B2B (Autoliquidation)

[vat_r2_sub]
0 % TVA — l'acheteur comptabilise la TVA conformément à l'Art. 44 Dir. 2006/112/CE

[vat_r3_label]
UE B2C (Règles OSS)

[vat_r3_sub]
Taux du vendeur appliqué (ou taux OSS le cas échéant)

[vat_r4_label]
Acheteur hors UE

[vat_r4_sub]
0 % TVA — hors du champ d'application de la TVA UE, exonéré

[vat_rates_title]
Taux de TVA pris en charge

[pricing_tag]
Tarifs

[pricing_title]
Des tarifs simples et honnêtes

[pricing_subtitle]
Commencez gratuitement. Passez au niveau supérieur quand votre activité se développe. Sans engagement, résiliation à tout moment.

[p_free_name]
Gratuit

[p_free_price]
€0

[p_free_period]
pour toujours

[p_free_desc]
Tout ce dont vous avez besoin pour démarrer.

[p_free_l1]
Jusqu'à 3 clients

[p_free_l2]
5 factures par mois

[p_free_l3]
Suivi du temps

[p_free_l4]
Export de factures PDF

[p_free_cta]
Commencer gratuitement

[p_starter_name]
Starter

[p_starter_price]
€9

[p_starter_period]
par mois

[p_starter_desc]
Pour les freelances en croissance.

[p_starter_l1]
Clients illimités

[p_starter_l2]
20 factures par mois

[p_starter_l3]
Suivi du temps

[p_starter_l4]
Toutes les règles TVA UE

[p_starter_l5]
Rappels de paiement

[p_starter_cta]
Choisir Starter

[p_pro_name]
Pro

[p_pro_price]
€29

[p_pro_period]
par mois

[p_pro_desc]
Pour les freelances sérieux.

[p_pro_l1]
Tout illimité

[p_pro_l2]
Factures récurrentes

[p_pro_l3]
Portail client

[p_pro_l4]
Peppol / Facturation électronique (UBL 2.1)

[p_pro_l5]
Mode exonération TVA (les 27 pays de l'UE)

[p_pro_l6]
Toutes les règles TVA UE

[p_pro_cta]
Passer au Pro

[p_popular]
Le plus populaire

[faq_tag]
FAQ

[faq_title]
Questions fréquemment posées

[faq1_q]
InvoiceKit est-il vraiment conforme à la TVA européenne ?

[faq1_a]
Oui. InvoiceKit calcule automatiquement la TVA correcte pour chaque facture en fonction de votre pays, du pays de votre client et de sa validité de numéro de TVA UE. Il gère correctement les taux normaux, l'autoliquidation, l'OSS et l'exonération hors UE.

[faq2_q]
Quels pays de l'UE sont pris en charge ?

[faq2_a]
Les 27 États membres de l'UE. Les taux de TVA sont codés pour BG, DE, FR, RO, PL, CZ, IT, ES, NL, PT, AT, BE, HR, HU, SE et tous les autres. Les taux sont mis à jour lors de changements législatifs.

[faq3_q]
InvoiceKit prend-il en charge les factures récurrentes ?

[faq3_a]
Oui. Vous pouvez transformer n'importe quelle facture en récurrente avec des intervalles hebdomadaires, mensuels, trimestriels ou annuels. InvoiceKit clone et émet automatiquement la facture à la date d'échéance — vous n'avez rien à faire.

[faq4_q]
Comment fonctionne le suivi du temps ?

[faq4_a]
Créez des projets avec des clients et des taux horaires. Utilisez le minuteur start/stop intégré pendant votre travail. Tout le temps suivi est stocké et peut être converti directement en lignes de facture en un clic.

[faq5_q]
Y a-t-il une période d'essai gratuite ?

[faq5_a]
Oui. Tous les nouveaux comptes démarrent avec un essai Pro de 14 jours — sans carte de crédit. Vous bénéficiez d'un accès Pro complet dès le premier jour. Après l'essai, vous restez sur le plan gratuit sauf si vous choisissez de passer à une version supérieure.

[cta_title]
Prêt à facturer à l'européenne ?

[cta_subtitle]
Rejoignez les freelances européens qui ont arrêté de se battre avec des tableurs et commencé à être payés à temps.

[cta_button]
Commencer gratuitement aujourd'hui →

[footer_tagline]
Facturation conforme UE pour les freelances européens.

[footer_privacy]
Politique de confidentialité

[footer_terms]
Conditions d'utilisation

[footer_copy]
© 2026 InvoiceKit. Tous droits réservés.
