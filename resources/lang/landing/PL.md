[nav_features]
Funkcje

[nav_pricing]
Cennik

[nav_login]
Zaloguj się

[nav_cta]
Zacznij za darmo

[hero_badge]
🇪🇺 Stworzony dla europejskich freelancerów

[hero_headline]
Fakturuj jak Europejczyk.
Śledź czas jak profesjonalista.

[hero_subheadline]
Jedyne narzędzie, którego potrzebują freelancerzy z UE — od śledzenia czasu po faktury VAT zgodne z prawem. Automatycznie obsługuje odwrotne obciążenie, OSS i wszystkie 24 stawki VAT w UE.

[hero_cta_primary]
Zacznij za darmo — bez karty

[hero_cta_secondary]
Zobacz funkcje ↓

[hero_trust_1]
Bezpłatny start

[hero_trust_2]
Bez karty kredytowej

[hero_trust_3]
Anuluj w dowolnym momencie

[features_tag]
Funkcje

[features_title]
Wszystko w jednym miejscu

[features_subtitle]
Przestań żonglować arkuszami kalkulacyjnymi i oddzielnymi aplikacjami. InvoiceKit obsługuje każdy krok od pierwszej rozliczanej minuty do ostatniego zebranego euro.

[f1_title]
Śledzenie czasu

[f1_desc]
Timer start/stop dla każdego projektu. Ręczne wprowadzanie dla wcześniejszej pracy. Dzienne dzienniki, tygodniowe podsumowania i miesięczne raporty z łącznymi godzinami do rozliczenia.

[f2_title]
Kreator faktur

[f2_desc]
Generuj profesjonalne faktury PDF w kilka sekund. Automatyczna numeracja, format prawny, ze wszystkimi wymaganymi polami UE, w tym pełnym podziałem VAT.

[f3_title]
Silnik VAT UE

[f3_desc]
Automatyczny VAT dla każdego scenariusza UE: stawka standardowa, odwrotne obciążenie (B2B), OSS (konsumenci B2C w UE) i 0% zwolnione dla nabywców spoza UE.

[f4_title]
Zarządzanie klientami

[f4_desc]
Przechowuj klientów z krajem, numerem VAT i walutą. Flagi krajów, natychmiastowe wyszukiwanie i pełna historia fakturowania na klienta.

[f5_title]
Faktury cykliczne

[f5_desc]
Skonfiguruj tygodniowe, miesięczne, kwartalne lub roczne faktury. InvoiceKit automatycznie klonuje i wysyła je — zero ręcznej pracy, zero pominiętych cykli rozliczeniowych.

[f6_title]
Portal klienta

[f6_desc]
Udostępnij bezpieczny tokenizowany link dowolnemu klientowi. Mogą przeglądać fakturę, pobierać PDF i płacić kartą online przez Stripe — środki trafiają bezpośrednio na Twoje konto bankowe. Lub udostępnij swój IBAN na bezpłatne przelewy bankowe. Bez logowania.

[f7_title]
Działa na każddym urządzeniu

[f7_desc]
Dodaj InvoiceKit do ekranu głównego telefonu, tabletu lub komputera. Instaluje się jak natywna aplikacja, uruchamia bez przeglądarki i działa offline, abyś mógł rejestrować czas lub sprawdzać faktury gdziekolwiek jesteś.

[f8_title]
Śledzenie wydatków

[f8_desc]
Rejestruj wydatki firmowe z paragonami, kategoriami i danymi dostawcy. Filtruj według daty lub kategorii, eksportuj do CSV i śledź miesięczne wydatki obok przychodów.

[f9_title]
Wiele walut

[f9_desc]
Wystawiaj faktury w EUR, USD, BGN, RON, PLN, CZK lub HUF. Każdy klient przechowuje preferowaną walutę — jest automatycznie uzupełniana na każdej nowej fakturze.

[how_tag]
Jak to działa

[how_title]
Od śledzonych sekund do opłaconej faktury

[how_subtitle]
Trzy proste kroki. To wszystko, czego potrzeba.

[s1_title]
Śledź swój czas

[s1_desc]
Kliknij Start dla projektu. Pracuj. Kliknij Stop. Każda sekunda jest rejestrowana automatycznie — bez arkuszy, bez zapominania.

[s2_title]
Wygeneruj fakturę

[s2_desc]
Wybierz klienta, zaznacz wpisy czasu do rozliczenia. InvoiceKit buduje fakturę z prawidłowym VAT, formatem prawnym i Twoim brandingiem.

[s3_title]
Otrzymaj płatność

[s3_desc]
Wyślij jednym kliknięciem. Twój klient płaci kartą przez Stripe lub bezpłatnym przelewem bankowym. Środki trafiają bezpośrednio na Twoje konto bankowe. Gotowe.

[vat_tag]
Zgodność z UE

[vat_title]
VAT prawidłowo za każdym razem

[vat_desc]
Prawo VAT w UE jest skomplikowane. InvoiceKit obsługuje każdy scenariusz automatycznie — stawki standardowe, odwrotne obciążenie dla klientów B2B, OSS dla konsumentów UE i 0% dla nabywców spoza UE.

[vat_r1_label]
Fakturowanie w tym samym kraju

[vat_r1_sub]
Lokalna standardowa stawka VAT stosowana automatycznie

[vat_r2_label]
UE B2B (Odwrotne obciążenie)

[vat_r2_sub]
0% VAT — nabywca rozlicza VAT zgodnie z art. 44 Dyrektywy 2006/112/WE

[vat_r3_label]
UE B2C (Zasady OSS)

[vat_r3_sub]
Stosowana stawka sprzedawcy (lub stawka OSS, jeśli dotyczy)

[vat_r4_label]
Nabywca spoza UE

[vat_r4_sub]
0% VAT — poza zakresem VAT UE, zwolnione

[vat_rates_title]
Obsługiwane stawki VAT

[pricing_tag]
Cennik

[pricing_title]
Prosty, uczciwy cennik

[pricing_subtitle]
Zacznij za darmo. Ulepsz, gdy Twoja firma rośnie. Bez umów, anuluj w dowolnym momencie.

[p_free_name]
Darmowy

[p_free_price]
€0

[p_free_period]
na zawsze

[p_free_desc]
Wszystko, czego potrzebujesz, aby zacząć.

[p_free_l1]
Do 3 klientów

[p_free_l2]
5 faktur miesięcznie

[p_free_l3]
Śledzenie czasu

[p_free_l4]
Eksport faktur PDF

[p_free_l6]
:free_limit importy AI dziennie

[p_free_cta]
Zacznij za darmo

[p_starter_name]
Starter

[p_starter_price]
€9

[p_starter_period]
miesięcznie

[p_starter_desc]
Dla rozwijających się freelancerów.

[p_starter_l1]
Nieograniczona liczba klientów

[p_starter_l2]
20 faktur miesięcznie

[p_starter_l3]
Śledzenie czasu

[p_starter_l4]
Wszystkie zasady VAT UE

[p_starter_l5]
Przypomnienia o płatnościach

[p_starter_l7]
:starter_limit importy AI dziennie

[p_starter_cta]
Wybierz Starter

[p_pro_name]
Pro

[p_pro_price]
€29

[p_pro_period]
miesięcznie

[p_pro_desc]
Dla poważnych freelancerów.

[p_pro_l1]
Wszystko bez ograniczeń

[p_pro_l2]
Faktury cykliczne

[p_pro_l3]
Portal klienta

[p_pro_l4]
Peppol / e-Fakturowanie (UBL 2.1)

[p_pro_l5]
Tryb zwolnienia z VAT (wszystkie 27 krajów UE)

[p_pro_l6]
Wszystkie zasady VAT UE

[p_pro_l7]
Płatności online przez Stripe (prowizja 2%)

[p_pro_l8]
Nieograniczone importy AI

[p_pro_cta]
Przejdź na Pro

[p_popular]
Najpopularniejszy

[faq_tag]
FAQ

[faq_title]
Często zadawane pytania

[faq1_q]
Czy InvoiceKit jest naprawdę zgodny z VAT UE?

[faq1_a]
Tak. InvoiceKit automatycznie oblicza prawidłowy VAT dla każdej faktury na podstawie Twojego kraju, kraju klienta i tego, czy posiada ważny numer VAT UE. Prawidłowo obsługuje stawki standardowe, odwrotne obciążenie, OSS i scenariusze zwolnień spoza UE.

[faq2_q]
Które kraje UE są obsługiwane?

[faq2_a]
Wszystkie 27 państw członkowskich UE. Stawki VAT są zakodowane dla BG, DE, FR, RO, PL, CZ, IT, ES, NL, PT, AT, BE, HR, HU, SE i wszystkich pozostałych. Stawki są aktualizowane po zmianach przepisów UE.

[faq3_q]
Czy InvoiceKit obsługuje faktury cykliczne?

[faq3_a]
Tak. Możesz zamienić dowolną fakturę w cykliczną z tygodniowymi, miesięcznymi, kwartalnymi lub rocznymi interwałami. InvoiceKit automatycznie klonuje i wystawia fakturę w terminie płatności — nie musisz nic robić.

[faq4_q]
Jak działa śledzenie czasu?

[faq4_a]
Twórz projekty z klientami i stawkami godzinowymi. Używaj wbudowanego timera start/stop podczas pracy. Cały śledzony czas jest przechowywany i można go jednym kliknięciem przekonwertować bezpośrednio na pozycje faktury.

[faq5_q]
Czy dostępny jest bezpłatny okres próbny?

[faq5_a]
Tak. Wszystkie nowe konta zaczynają z 14-dniowym okresem próbnym Pro — karta kredytowa nie jest wymagana. Od pierwszego dnia masz pełny dostęp do Pro. Po zakończeniu okresu próbnego pozostajesz na planie darmowym, chyba że zdecydujesz się na ulepszenie.


[pay_tag]
Płatności online

[pay_title]
Otrzymuj płatności bezpośrednio. Zawsze.

[pay_desc]
Połącz swoje konto Stripe i zamień każdą fakturę w link do płatności. Klienci płacą kartą — środki trafiają bezpośrednio na Twoje konto bankowe, nie do pośrednika. Przelewy bankowe przez IBAN są zawsze bezpłatne.

[pay_r1_label]
Stripe Connect — bezpośrednio na Twoje konto

[pay_r1_sub]
Używamy Stripe Connect Express. Płatność kartą Twojego klienta trafia bezpośrednio na Twoje konto bankowe. InvoiceKit nigdy nie przetrzymuje Twoich pieniędzy.

[pay_r2_label]
Portal klienta — bez logowania

[pay_r2_sub]
Każda faktura otrzymuje bezpieczny tokenizowany link. Twój klient otwiera go w dowolnej przeglądarce, widzi fakturę i płaci jednym kliknięciem.

[pay_r3_label]
Tylko 2% za płatności kartą — przelewy bezpłatne

[pay_r3_sub]
2% prowizja platformy dotyczy tylko płatności kartą. Przelewy bankowe przez IBAN są całkowicie bezpłatne.

[faq6_q]
Jak działają płatności online?

[faq6_a]
Połącz swoje konto Stripe w Ustawieniach → Płatności. InvoiceKit generuje bezpieczny link do płatności dla każdej faktury. Twój klient klika Zapłać teraz i płaci kartą. Środki trafiają bezpośrednio na Twoje konto bankowe — InvoiceKit nigdy nie przetrzymuje Twoich pieniędzy. Do płatności kartą stosowana jest prowizja platformy 2%. Przelewy bankowe przez IBAN są zawsze bezpłatne.

[faq7_q]
Jak działa import dokumentów AI?

[faq7_a]
InvoiceKit może odczytywać faktury i dokumenty wydatków (PDF, JPEG, PNG) i automatycznie wyodrębniać dostawcę, kwotę, datę i pozycje za pomocą Google Gemini AI. Użytkownicy planu darmowego otrzymują :free_limit importy AI dziennie; planu Starter — :starter_limit. Użytkownicy Pro mają nieograniczoną liczbę importów. W każdym planie możesz dodać własny bezpłatny klucz Gemini API w ustawieniach Ustawienia → AI, aby usunąć wszystkie ograniczenia.

[p_byok_label]
Użyj własnego klucza API Gemini — bez limitów

[p_byok_desc]
W każdym planie możesz dodać własny bezpłatny klucz Google Gemini API w ustawieniach Ustawienia → AI. Po skonfigurowaniu wszystkie importy dokumentów AI korzystają z Twojego limitu Google — żadne limity aplikacji nie mają zastosowania.

[cta_title]
Gotowy do fakturowania jak Europejczyk?

[cta_subtitle]
Dołącz do europejskich freelancerów, którzy skończyli walczyć z arkuszami kalkulacyjnymi i zaczęli otrzymywać płatności na czas.

[cta_button]
Zacznij za darmo dziś →

[footer_tagline]
Fakturowanie zgodne z UE dla europejskich freelancerów.

[footer_privacy]
Polityka prywatności

[footer_terms]
Warunki usługi

[footer_copy]
© 2026 NetShell. Wszelkie prawa zastrzeżone.

[f10_title]
Import dokumentów przez AI

[f10_desc]
Prześlij faktury, paragony lub dokumenty wydatków. AI automatycznie wyodrębnia kwoty, daty, dostawców i VAT. Sprawdź i zapisz jednym kliknięciem.

[ai_import_tag]
Import wspomagany AI

[ai_import_title]
Pozwól AI zająć się papierologią

[ai_import_desc]
Prześlij PDF, JPG lub PNG dowolnego dokumentu. Gemini AI odczytuje i wyodrębnia wszystkie dane za Ciebie. Sprawdź wyodrębnione pola, dostosuj w razie potrzeby i zapisz jako wersję roboczą faktury lub wydatek jednym kliknięciem.

[ai_import_r1_label]
PDF, JPG i PNG obsługiwane

[ai_import_r1_sub]
Prześlij do 10 plików naraz — przetwarzane równolegle przez AI

[ai_import_r2_label]
Pełna automatyczna ekstrakcja

[ai_import_r2_sub]
Kwoty, daty, numery VAT, nazwy dostawców i pozycje — wyodrębniane natychmiastowo

[ai_import_r3_label]
Weryfikacja przed zapisem

[ai_import_r3_sub]
Każdy import przechodzi przez ekran potwierdzenia — zachowujesz kontrolę