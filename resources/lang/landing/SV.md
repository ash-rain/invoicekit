[nav_features]
Funktioner

[nav_pricing]
Priser

[nav_login]
Logga in

[nav_cta]
Kom igång gratis

[hero_badge]
🇪🇺 Byggt för europeiska frilansare

[hero_headline]
Fakturera som en europé.
Spåra tid som ett proffs.

[hero_subheadline]
Det enda verktyget EU-frilansare behöver — från tidsspårning till lagligt korrekta momsfakturor. Hanterar automatiskt omvänd skattskyldighet, OSS och alla 24 EU-momssatser.

[hero_cta_primary]
Kom igång gratis — inget kort

[hero_cta_secondary]
Se funktioner ↓

[hero_trust_1]
Gratis att börja

[hero_trust_2]
Inget kreditkort behövs

[hero_trust_3]
Avsluta när som helst

[features_tag]
Funktioner

[features_title]
Allt på ett ställe

[features_subtitle]
Sluta jonglera med kalkylblad och separata appar. InvoiceKit hanterar varje steg från din första fakturerbara minut till din sista insamlade euro.

[f1_title]
Tidsspårning

[f1_desc]
Start/stopp-timer per projekt. Manuell inmatning för tidigare arbete. Dagliga loggar, veckosammanfattningar och månadsrapporter med totala fakturerbara timmar.

[f2_title]
Fakturabyggare

[f2_desc]
Generera professionella PDF-fakturor på sekunder. Automatisk numrering, juridisk formatering, med alla obligatoriska EU-fält inklusive fullständig momsuppdelning.

[f3_title]
EU-momsmotor

[f3_desc]
Automatisk moms för varje EU-scenario: normalskattesats, omvänd skattskyldighet (B2B), OSS (B2C EU-konsumenter) och 0% momsfri för köpare utanför EU.

[f4_title]
Kundhantering

[f4_desc]
Lagra kunder med land, momsnummer och valuta. Landsflaggor, omedelbar sökning och komplett faktureringshistorik per kund.

[f5_title]
Återkommande fakturor

[f5_desc]
Ställ in veckovisa, månatliga, kvartalsvisa eller årliga fakturor. InvoiceKit klonar och skickar dem automatiskt — inget manuellt arbete, inga missade faktureringscykler.

[f6_title]
Kundportal

[f6_desc]
Dela en säker tokeniserad länk med vilken klient som helst. De kan se fakturan, ladda ner PDF:en och betala online med kort via Stripe — pengarna går direkt till ditt bankkonto. Eller dela ditt IBAN för gratis banköverföringar. Ingen inloggning krävs.

[f7_title]
Fungerar på alla enheter

[f7_desc]
Lägg till InvoiceKit på startskärmen på din telefon, surfplatta eller dator. Installeras som en native-app, startar utan webbläsare och fungerar offline, så att du kan logga tid eller kontrollera fakturor var som helst.

[f8_title]
Utgiftsspårning

[f8_desc]
Logga affärsutgifter med kvitton, kategorier och leverantörsuppgifter. Filtrera efter datum eller kategori, exportera till CSV och håll koll på månatliga utgifter bredvid dina intäkter.

[f9_title]
Flera valutor

[f9_desc]
Fakturera i EUR, USD, BGN, RON, PLN, CZK eller HUF. Varje kund lagrar sin föredragna valuta — den fylls automatiskt i på varje ny faktura.

[how_tag]
Hur det fungerar

[how_title]
Från spårade sekunder till betald faktura

[how_subtitle]
Tre enkla steg. Det är allt som krävs.

[s1_title]
Spåra din tid

[s1_desc]
Klicka Start för ett projekt. Arbeta. Klicka Stopp. Varje sekund loggas automatiskt — inga kalkylblad, inget glömmer.

[s2_title]
Generera faktura

[s2_desc]
Välj en kund, markera tidsposter att fakturera. InvoiceKit bygger fakturan med korrekt moms, juridisk formatering och din varumärkesbild.

[s3_title]
Bli betald

[s3_desc]
Skicka med ett klick. Din klient betalar med kort via Stripe eller med gratis banköverföring. Pengarna hamnar direkt på ditt bankkonto. Klart.

[vat_tag]
EU-efterlevnad

[vat_title]
Moms rätt varje gång

[vat_desc]
EU:s momslagstiftning är komplex. InvoiceKit hanterar varje scenario automatiskt — normalskattesatser, omvänd skattskyldighet för B2B-kunder, OSS för EU-konsumenter och 0% för köpare utanför EU.

[vat_r1_label]
Fakturering i samma land

[vat_r1_sub]
Lokal standardmomssats tillämpas automatiskt

[vat_r2_label]
EU B2B (Omvänd skattskyldighet)

[vat_r2_sub]
0% moms — köparen redovisar moms enl. Art. 44 Dir. 2006/112/EG

[vat_r3_label]
EU B2C (OSS-regler)

[vat_r3_sub]
Säljarens momssats tillämpas (eller OSS-sats där tillämpligt)

[vat_r4_label]
Köpare utanför EU

[vat_r4_sub]
0% moms — utanför EU-momsen tillämpningsområde, undantaget

[vat_rates_title]
Stödda momssatser

[pricing_tag]
Priser

[pricing_title]
Enkla, ärliga priser

[pricing_subtitle]
Börja gratis. Uppgradera när ditt företag växer. Inga kontrakt, avsluta när som helst.

[p_free_name]
Gratis

[p_free_price]
€0

[p_free_period]
för alltid

[p_free_desc]
Allt du behöver för att komma igång.

[p_free_l1]
Upp till 3 kunder

[p_free_l2]
5 fakturor per månad

[p_free_l3]
Tidsspårning

[p_free_l4]
PDF-fakturaexport

[p_free_cta]
Kom igång gratis

[p_starter_name]
Starter

[p_starter_price]
€9

[p_starter_period]
per månad

[p_starter_desc]
För växande frilansare.

[p_starter_l1]
Obegränsade kunder

[p_starter_l2]
20 fakturor per månad

[p_starter_l3]
Tidsspårning

[p_starter_l4]
Alla EU-momsregler

[p_starter_l5]
Betalningspåminnelser

[p_starter_cta]
Välj Starter

[p_pro_name]
Pro

[p_pro_price]
€29

[p_pro_period]
per månad

[p_pro_desc]
För seriösa frilansare.

[p_pro_l1]
Allt obegränsat

[p_pro_l2]
Återkommande fakturor

[p_pro_l3]
Kundportal

[p_pro_l4]
Peppol / e-Fakturering (UBL 2.1)

[p_pro_l5]
Momsbefrielseläge (alla 27 EU-länder)

[p_pro_l6]
Alla EU-momsregler

[p_pro_l7]
Onlinebetalningar via Stripe (2% avgift)

[p_pro_cta]
Gå till Pro

[p_popular]
Mest populär

[faq_tag]
FAQ

[faq_title]
Vanliga frågor

[faq1_q]
Är InvoiceKit verkligen EU-momskompatibelt?

[faq1_a]
Ja. InvoiceKit beräknar automatiskt rätt moms för varje faktura baserat på ditt land, din kunds land och om de har ett giltigt EU-momsnummer. Det hanterar korrekt normalskattesatser, omvänd skattskyldighet, OSS och icke-EU-undantagna scenarier.

[faq2_q]
Vilka EU-länder stöds?

[faq2_a]
Alla 27 EU-medlemsstater. Momssatser är hårdkodade för BG, DE, FR, RO, PL, CZ, IT, ES, NL, PT, AT, BE, HR, HU, SE och alla andra. Satser uppdateras när EU-lagstiftningen ändras.

[faq3_q]
Stöder InvoiceKit återkommande fakturor?

[faq3_a]
Ja. Du kan göra om valfri faktura till en återkommande med veckovisa, månatliga, kvartalsvisa eller årliga intervaller. InvoiceKit klonar och utfärdar fakturan automatiskt på förfallodatum — du behöver inte göra något.

[faq4_q]
Hur fungerar tidsspårning?

[faq4_a]
Skapa projekt med kunder och timtaxor. Använd den inbyggda start/stopp-timern medan du arbetar. All spårad tid sparas och kan med ett klick direkt konverteras till fakturarader.

[faq5_q]
Finns det en gratis provperiod?

[faq5_a]
Ja. Alla nya konton börjar med en 14-dagars Pro-provperiod — inget kreditkort krävs. Du har full tillgång till Pro från första dagen. Efter provperioden stannar du på gratisplanen såvida du inte väljer att uppgradera.


[pay_tag]
Onlinebetalningar

[pay_title]
Ta emot betalningar direkt. Alltid.

[pay_desc]
Koppla ditt Stripe-konto och gör varje faktura till en betalningslänk. Klienter betalar med kort — pengarna hamnar direkt på ditt bankkonto, inte hos en mellanhand. Banköverföringar via IBAN är alltid gratis.

[pay_r1_label]
Stripe Connect — direkt till din bank

[pay_r1_sub]
Vi använder Stripe Connect Express. Din klients kortbetalning går direkt till ditt bankkonto. InvoiceKit håller aldrig inne dina pengar.

[pay_r2_label]
Kundportalen — ingen inloggning

[pay_r2_sub]
Varje faktura får en säker tokeniserad länk. Din klient öppnar den i vilken webbläsare som helst, ser fakturan och betalar med ett klick.

[pay_r3_label]
Bara 2% för kortbetalningar — banköverföringar gratis

[pay_r3_sub]
En plattformsavgift på 2% gäller bara kortbetalningar. Banköverföringar via IBAN är helt gratis.

[faq6_q]
Hur fungerar onlinebetalningar?

[faq6_a]
Koppla ditt Stripe-konto under Inställningar → Betalningar. InvoiceKit genererar en säker betalningslänk för varje faktura. Din klient klickar på Betala nu och betalar med kort. Pengarna hamnar direkt på ditt bankkonto — InvoiceKit håller aldrig inne dina pengar. En plattformsavgift på 2% gäller för kortbetalningar. Banköverföringar via IBAN är alltid gratis.

[cta_title]
Redo att fakturera som en europé?

[cta_subtitle]
Gå med europeiska frilansare som slutat kämpa med kalkylblad och börjat bli betalda i tid.

[cta_button]
Börja gratis idag →

[footer_tagline]
EU-anpassad fakturering för europeiska frilansare.

[footer_privacy]
Integritetspolicy

[footer_terms]
Användarvillkor

[footer_copy]
© 2026 InvoiceKit. Alla rättigheter förbehållna.

[f10_title]
AI-dokumentimport

[f10_desc]
Ladda upp fakturor, kvitton eller utgiftsdokument. AI extraherar automatiskt belopp, datum, leverantörer och moms. Granska och spara med ett klick.

[ai_import_tag]
AI-driven import

[ai_import_title]
Låt AI hantera pappersarbetet

[ai_import_desc]
Ladda upp en PDF, JPG eller PNG av vilket dokument som helst. Gemini AI läser och extraherar all data åt dig. Granska de extraherade fälten, justera vid behov och spara som fakturautkast eller utgift med ett klick.

[ai_import_r1_label]
PDF, JPG & PNG stöds

[ai_import_r1_sub]
Ladda upp upp till 10 filer samtidigt — bearbetas parallellt av AI

[ai_import_r2_label]
Fullständig automatisk extrahering

[ai_import_r2_sub]
Belopp, datum, momsregistreringsnummer, leverantörsnamn och rader — extraheras omedelbart

[ai_import_r3_label]
Granska innan du sparar

[ai_import_r3_sub]
Varje import går genom en bekräftelseskärm — du har kontrollen